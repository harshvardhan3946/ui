        ...
        .controller('AmbulanceCtrl', function ($scope, $http, $compile, $mdToast) {
            ...
            $scope.enquire = function (ambulanceId) {
                ...
                if (formElem) {
                    ...
                    newElement.innerHTML = '<fieldset><legend><label>Enter Details</label></legend><br/><form name="enqform" ng-submit="enqform.$valid && book(\'' + ambulanceId + '\')" novalidate><span ng-show="enqform.$submitted && enqform.$invalid" style="color: #f4151a">All Fields are mandatory.</span><br/><span ng-show="enqform.$submitted && enqform.email.$error.email" style="color: #f4151a">Invalid Email address.</span><br/><input type="text" id="name" placeholder="Name" ng-model="name" required> <input type="email" name="email" id="email" placeholder="Email" ng-model="email" required> <input type="text" maxlength="12" id="contact" placeholder="Phone Number" ng-model="contact" required><button type="submit" class="shadow">PROCEED</button></form></fieldset>';
